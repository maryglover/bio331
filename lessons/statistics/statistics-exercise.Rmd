...
You can use this [template script](./statistics_script.R) file to work through the lesson. 
...
```{r, include = F, eval = F, fig.show=F}

head(wq)
do_temp.lm <- lm(data = wq, do_percent_sat ~ Temperature_C )
summary(do_temp.lm)
library(ggplot2)
ggplot(wq, aes(x = Temperature_C, y = do_percent_sat)) +
  geom_point() +
  geom_smooth(method = 'lm')
```
...
Here, we do so that the p-value is quite small, 7.8e-12, providing evidence that the streams do differ in Turbidity. 
Again, we can look at this with a graph
...
Go back to your Part 2 from your homework in the [advanced R lesson](https://maryglover.github.io/bio331/lessons/advanced-R/advancedR-exercise.html#Part_2:_Water_quality_and_precipitation). In this exercise, you will use the data that you created, were you joined the precipitation data to the water quality data. 
...
1. Summarize the result of the statistical test. Is the result significant? If so, is the relationship positive or negative?
1. Make a graph of the relationship between precipiation and turbidity. Make it pretty!
1. If the result was significant, add a best fit line to the graph. 
1. Write a 3-part figure caption with 1) A descriptive title 2) a methods statement and 3) a summary of the main result. 
...
joined <- stream_precip |>
  right_join(wq) 
precip.lm <- lm(data = joined, Turbidity_NTU~precip)
summary(precip.lm)
