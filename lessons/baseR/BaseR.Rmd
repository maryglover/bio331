...
For example, `log` is a function that will take the natural log of the value given as an argument, x.
...
When you use are reading in data files, R will look for the file in the folder or "directory" you are in. If the file is not in the home folder, you will need to specify where R should look. Folders in names are separated by "/". So, for example the directory "Documents/bio_files/" is a folder "bio_files" inside the folder "Documents." "Documents/bio_files/photosynthesis.pdf" ss a PDF file inside the bio_files folder. 
...
**Question**: Look back at your data from the first day of class. Is the data tidy? Do you need to reformat it?
...
**Complete these exercises in the Learning R project**. For the exercises, you should do all of your code in an R script. You will then export the R script and submit in the Moodle assignment. 
...
When you have completed the homework assignment. Export both the R script you used in class and the one you used for the homework. (It is OK if you only used one script file.)
